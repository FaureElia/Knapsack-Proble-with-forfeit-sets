...
def funzioneObiettivoFinale(lista):
    ...
    print("tot violazioni: ", tot_violazioni)
...
lista_temporanea=miglior_soluzione_trovata
# A QUESTO PUNTO VOGLIO PORVARE A SOSTITUIRE GLI ITEMS!
while(flagStop!=True and time.perf_counter()-start<400):
    #miglior_soluzione_trovata=list(lista_temporanea)
    flagStop=True
    for i in miglior_soluzione_trovata:
        for r in rimanenti:
            if (weights[r]<=spazio_libero+weights[i]):#ci sta l'item!!
                risultato=calcolaFunzioneObiettivo(miglior_soluzione_trovata,i,r,best_bound)
                if (risultato!=-1):
                    #in questo caso soluzione migliorata, esco dal ciclo e aggiorno
                    print("soluzione migliorata, tolgo ",i," metto ",r)
                    print(risultato)
                    best_bound=risultato
                    miglior_soluzione_trovata.append(r)
                    miglior_soluzione_trovata.remove(i)
                    cambiamento=[i,r]
                    aggiornoViolazioni(cambiamento)#aggiorno violazioni
                    spazio_libero=spazio_libero+weights[cambiamento[0]]-weights[cambiamento[1]] #aggiorno spazio
                    #aggiorno i rimanenti
                    rimanenti.remove(r)
                    rimanenti.append(i)
                    #
                    #memorizzo che è avvenuto un cambiamento
                    flagStop=False
                    #interrompo il ciclo
                    break
        if (not flagStop):#se fagStop=False, cioè ho aggionrato il valore, interrompo ciclo e rinizio
            break;


end=time.perf_counter()
print("termine algoritmo, TEMPO impiegato: ",(end-start))
...
print("spazio occupato: ", capacity-spazio_libero,",gurobi ",spazio_occupato," su capacità di ", capacity)
print("tempo impiegato: ", end-start)
